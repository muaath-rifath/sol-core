...
static char *read_slot(const esp_partition_t *partition, uint32_t offset) {
  ...
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "Failed to read partition at offset 0x%lx: %s", offset,
             esp_err_to_name(err));
    free(buf);
    return NULL;
  }

  // Ensure null termination. We check if the data starts with standard PEM
  // headers. If the partition is empty (all 0xFF), we return NULL.
  if ((uint8_t)buf[0] == 0xFF) {
    free(buf);
    return NULL;
  }

  // The partition might not be fully filled, ensure it's null terminated within
  // SLOT_SIZE
...
esp_err_t certs_load(cert_bundle_t *bundle) {
  const esp_partition_t *partition = esp_partition_find_first(
      ESP_PARTITION_TYPE_DATA, 0x99, CERT_PARTITION_LABEL);
  if (!partition) {
    ESP_LOGW(TAG, "Certificates partition '%s' not found",
