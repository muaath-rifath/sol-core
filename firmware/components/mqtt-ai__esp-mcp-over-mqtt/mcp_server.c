...
static bool mcp_server_tool_check(mcp_server_t *server, const char *tool_name,
                                  ...
                                  mcp_tool_t **tool)
{
    if (server == NULL || tool_name == NULL || n_args < 0 || tool == NULL) {
                          ...
                          void *event_data)
{
    ...
    case MQTT_EVENT_DISCONNECTED:
        ESP_LOGW("mcp_server", "MQTT client disconnected");
        // Clear stale client list so RPC topics are re-subscribed on reconnect.
        // insert_client() skips subscription for already-known clients, so
        // without this clear the ESP32 never resubscribes to RPC topics after
        // the session expires and tools/call messages are silently dropped.
        for (int i = 0; i < server->n_clients; i++) {
            free(server->clients[i].client_id);
        }
        if (server->n_clients > 0) {
    ...
    case MQTT_EVENT_ERROR:
        ESP_LOGE("mcp_server", "MQTT client error event");
        break;
